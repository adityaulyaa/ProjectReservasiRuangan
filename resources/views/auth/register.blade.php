...
<head>
    ...
    <title>Daftar Sekarang - AksesRuang</title>
    @vite(['resources/css/app.css', 'resources/js/app.js'])
</head>
<body class="font-sans antialiased overflow-x-hidden">
    <div class="relative min-h-screen lg:h-screen lg:overflow-hidden flex items-start lg:items-start justify-center py-6 lg:py-0 overflow-y-auto lg:overflow-y-hidden">
        ...
        <div class="relative flex w-full max-w-6xl mx-auto px-4 sm:px-6 gap-6 lg:gap-8 flex-col lg:flex-row items-center lg:items-start justify-center lg:justify-center h-full lg:pt-8 xl:pt-12">
            ...
            <div class="hidden lg:flex flex-1 flex-col text-white pt-6 xl:pt-8 pb-4 animate-fade-in-left" style="animation-fill-mode: both;">
                ...
                <h1 class="text-3xl lg:text-4xl xl:text-5xl font-black mb-3 leading-none tracking-tight animate-fade-in-up stagger-2" style="animation-fill-mode: both;">
                    BERGABUNG<br>
                    DENGAN<br>
                ...
                <p class="text-sm xl:text-base mb-6 opacity-90 leading-relaxed animate-fade-in-up stagger-3" style="animation-fill-mode: both;">
                    Bergabung dengan Platform Booking<br>
                    Ruang Rapat &amp; Studi favorit kampus
                </p>
            ...
            <div class="w-full max-w-sm sm:max-w-md lg:max-w-sm lg:self-center mobile-full-card animate-fade-in-right lg:pt-2" style="animation-fill-mode: both;">
                <div class="glass-shimmer backdrop-blur-md bg-white/10 border border-white/20 rounded-3xl p-5 sm:p-6 shadow-2xl relative z-10">
                    ...
                    <div class="animate-fade-in-up stagger-1" style="animation-fill-mode: both;">
                        <h2 class="text-xl font-bold text-white">Daftar Sekarang</h2>
                        <p class="text-xs text-white/60 mt-1 mb-4">Isi data di bawah — akun menunggu verifikasi admin</p>
                    </div>
                    ...
                    <form method="POST" action="{{ route('register') }}" x-data="registerForm()" @submit="validateForm" class="relative z-10">
                        @csrf

                        <div class="mb-3 animate-fade-in-up stagger-2" style="animation-fill-mode: both;">
                            <label for="name" class="block text-xs font-semibold text-white/90 mb-1.5">Nama Lengkap</label>
                            <div class="relative group">
                                ...
                                <input id="name" type="text" name="name" value="{{ old('name') }}" placeholder="Nama Lengkap"
                                       x-model="name" @input="validateName()" required
                            ...
                            <template x-if="errors.name"><p class="text-red-300 text-xs mt-1" x-text="errors.name"></p></template>
                        </div>
                        ...
                        <div class="mb-3 animate-fade-in-up stagger-4" style="animation-fill-mode: both;">
                            ...
                            <div class="relative group">
                                ...
                                <button type="button" @click="showPassword = !showPassword" class="absolute right-3 top-2.5 text-white/50 hover:text-white transition">
                                    <svg x-show="!showPassword" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.878 9.878L5.636 5.636m4.242 4.242L14.12 14.12m-4.242-4.242L19.5 5.636M14.12 14.12L19.5 19.5M9.878 9.878L3 3m6.878 6.878L21 21"/><text x="0" y="0" /></svg>
                        ...
                        <div class="mb-4 animate-fade-in-up stagger-5" style="animation-fill-mode: both;">
                            <label for="password_confirmation" class="block text-xs font-semibold text-white/90 mb-1.5">Konfirmasi Kata Sandi</label>
                            <div class="relative group">
                                <svg class="absolute left-3 top-2.5 w-4 h-4 text-white/50 group-focus-within:text-teal-300 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                                </svg>
                                <input id="password_confirmation" :type="showPasswordConfirm ? 'text' : 'password'" name="password_confirmation" placeholder="Konfirmasi Kata Sandi"
                                       x-model="passwordConfirmation" @input="validatePassword()" required
                                       class="input-glow w-full pl-10 pr-10 py-2.5 bg-white/15 border border-white/25 rounded-xl text-white placeholder-white/50 focus:outline-none transition-all duration-300 text-sm">
                                <button type="button" @click="showPasswordConfirm = !showPasswordConfirm" class="absolute right-3 top-2.5 text-white/50 hover:text-white transition">
                                    <svg x-show="!showPasswordConfirm" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.878 9.878L5.636 5.636m4.242 4.242L14.12 14.12m-4.242-4.242L19.5 5.636M14.12 14.12L19.5 19.5M9.878 9.878L3 3m6.878 6.878L21 21"/></svg>
                                    <svg x-show="showPasswordConfirm" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                </button>
                            </div>
                        </div>
                        ...
                        <div class="p-3 bg-teal-500/15 border border-teal-400/30 rounded-xl backdrop-blur animate-fade-in-up stagger-7" style="animation-fill-mode: both;">
                            <p class="text-xs text-white/90 leading-relaxed">
                                <span class="font-semibold text-teal-200">Catatan:</span> Setelah mendaftar, akun Anda menunggu verifikasi admin.
                            </p>
                        </div>
                        ...
                        <div class="mt-3 text-center animate-fade-in-up stagger-8" style="animation-fill-mode: both;">
                            <p class="text-white/70 text-xs">
                                Sudah punya akun?
                                <a href="{{ route('login') }}" class="text-teal-300 hover:text-teal-200 font-bold transition underline underline-offset-4 decoration-teal-300/40 hover:decoration-teal-300">Masuk di sini</a>
                            </p>
                        </div>
                    </form>
                </div>
    ...
    <script>
        function registerForm() {
            return {
                ...
                errors: {},
                get isFormValid() {
                    return this.name && this.email && this.password && this.passwordConfirmation && this.password === this.passwordConfirmation && !this.errors.name && !this.errors.email && !this.errors.password;
                },
                validateName() {
                    this.errors.name = '';
                ...
                validatePassword() {
                    ...
                    else if (this.password !== this.passwordConfirmation && this.passwordConfirmation) this.errors.password = 'Konfirmasi kata sandi tidak cocok';
                },
                validateForm(e) {
                    this.validateName(); this.validateEmail(); this.validatePassword();
